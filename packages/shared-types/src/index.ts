...
export interface SendOtpResponse {
  success: boolean;
  message: string;
  devCode?: string; // Included only in dev mode for easy testing
}
...
export interface RegisterKeysRequest {
  ...
  identityPublicKey: string; // Base64 exportable EC public key
  signedPreKey: {
    keyId: number;
    publicKey: string;
    signature: string;
...
export interface SendMessageRequest {
  ...
  mediaUrl?: string;
  mediaKey?: string; // Encrypted symmetric media key
  replyToId?: string;
}
